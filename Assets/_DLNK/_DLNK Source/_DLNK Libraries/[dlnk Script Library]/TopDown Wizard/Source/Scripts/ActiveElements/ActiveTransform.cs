...
public class ActiveTransform : MonoBehaviour
{
    ...
    public class Target
    {
        ...
        public Quaternion RotateGO = new Quaternion(0, 0, 0, 100);
    ...
    void Start()
    {
            ...
            foreach (Target tar in Targets)
        {
            ...
            tar.originalTf = tar.TargetGO.transform.position;
            // save start scale
            if (tar.Scalable)
            {
            ...
            {
                // Move position
                tar.TargetGO.transform.position = (tar.originalTf + tar.TranslateGO);
                // Rotate
                tar.TargetGO.transform.rotation = new Quaternion(tar.RotateGO.x, tar.RotateGO.y, tar.RotateGO.z, tar.RotateGO.w);
                // Scale
                if (tar.Scalable)
    ...
    void Update()
    {
        ...
        if (_ismoving & !_waitforit)
        {
            ...
            {
                // Move position
                tar.TargetGO.transform.position = tar.originalTf + (_percentage * tar.TranslateGO);
                // Rotate
